...
// Atomic: updates Application.status AND creates a StatusHistory record in one transaction.
router.patch('/:id/status', async (req, res, next) => {
  try {
    ...
    // Fetch current record so we can record old_status and check applied_at
    ...
    const application = await prisma.$transaction(async (tx) => {
      const updated = await tx.application.update({
        ...
        data: {
          status,
          // Auto-stamp applied_at the first time we move to APPLIED
